...
(* NOTE(Linnea, 2021-04-05): Edits from original: use List and Array instead of
   Ext_list and Ext_array. *)

let is_nullary_variant (x : Types.constructor_arguments) =
  match x with Types.Cstr_tuple [] -> true | _ -> false

let array_reverse_of_list lst = lst |> List.rev |> Array.of_list
...
let names_from_construct_pattern (pat : Typedtree.pattern) =
  let names_from_type_variant (cstrs : Types.constructor_declaration list) =
    ...
    in
    Some
      { Lambda.consts= array_reverse_of_list consts
      ; blocks= array_reverse_of_list blocks }
  in
  let rec resolve_path n (path : Path.t) =
    ...
    | {type_kind= Type_abstract; type_manifest= Some t; _} -> (
      ...
      | Tconstr (pathn, _, _) ->
          (* Format.eprintf "XXX path%d:%s path%d:%s@." n (Path.name path) (n + 1)
           *   (Path.name pathn) ; *)
